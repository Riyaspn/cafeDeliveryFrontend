-- ============================================================
-- V8__add_org_id_to_addresses_and_notifications.sql
-- CafeQR Delivery — Add org_id to delivery_addresses and
-- delivery_notifications_log for proper branch-level scoping.
--
-- Why:
...
ALTER TABLE public.delivery_addresses
    ADD COLUMN IF NOT EXISTS org_id UUID;  -- FK → organizations.id (branch)
...
COMMENT ON COLUMN public.delivery_addresses.org_id
    IS 'Branch (organizations.id) this address was saved for. NULL = legacy rows before V8 migration.';
...
COMMENT ON COLUMN public.delivery_notifications_log.org_id
    IS 'Branch (organizations.id) the notification was sent for. NULL = legacy rows before V8 migration.';
