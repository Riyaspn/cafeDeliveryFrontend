 ...
 * /order?r=<org_uuid>&t=DELIVERY
 *
 * The `r` param carries the branch (organizations.id / org_id),
 * NOT the client/restaurant top-level id.
 * Each branch has its own geolocation, delivery radius, and settings.
 */
export function parseOrderParams(query) {
  const orgId     = query?.r || null;   // organizations.id (branch)
  const orderType = query?.t || 'DELIVERY'; // DELIVERY | TAKEAWAY
  return { orgId, orderType };
}
...
/**
 * Build delivery order payload for the backend API.
 *
 * Note: clientId is NOT taken from the URL param anymore.
 * The backend derives clientId from orgId (organizations.clientid)
 * so the frontend only needs to pass orgId.
 */
export function buildDeliveryOrderPayload({
  orgId, cartItems, customer, address,
